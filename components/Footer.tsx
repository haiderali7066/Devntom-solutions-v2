...
function LiveClock() {
  ...
  return <span className="font-black text-white tabular-nums">{time}</span>;
}
...
function VioletBlob() {
  ...
  useAnimationFrame((_, delta) => {
    ...
    const blobs = [
      ...
      { x: 0.82 + 0.05 * Math.cos(t * 0.9), y: 0.55 + 0.09 * Math.sin(t * 1.3), r: 0.30, a: 0.40, c: "14,165,233"  },  // sky
      { x: 0.60 + 0.10 * Math.sin(t * 0.6), y: 0.28 + 0.08 * Math.cos(t * 1.5), r: 0.24, a: 0.30, c: "99,102,241"  },  // indigo
...
export default function Footer() {
  return (
    <footer className="relative bg-[#080810] overflow-hidden font-sans">
      ...
      <div className="relative z-10 border-b border-white/[0.07]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-5">
            {SOCIALS.map((s) => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-[12px] font-semibold text-white/35 hover:text-white transition-colors duration-200">
          ...
          <a href="mailto:devntomsolutions@gmail.com"
            className="text-[12px] font-semibold text-white/35 hover:text-white transition-colors tracking-tight">
      ...
      {/* ── Main grid ── */}
      <div className="relative z-10 border-b border-white/[0.07]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-14 grid grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1fr_1fr] gap-10 lg:gap-8">
          ...
          <div className="col-span-2 lg:col-span-1 flex flex-col gap-7">
            ...
            <Link href="/" className="flex items-center gap-2 group">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center group-hover:scale-105 transition-transform"
                style={{ background: "linear-gradient(135deg, #0ea5e9, #7c3aed)", boxShadow: "0 4px 18px rgba(14,165,233,0.4)" }}>
                <span className="text-sm font-black text-white">D</span>
              </div>
              <span className="text-base font-black tracking-tight text-white">DEVNTOM<span className="text-sky-400">.</span></span>
            </Link>

            {/* Stacked nav links */}
            <nav className="flex flex-col gap-0.5">
              {NAV_LINKS.map((link, i) => (
                <motion.div key={link.label} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }} transition={{ delay: 0.06 * i }}>
                  <Link href={link.href}
                    className={`block text-[1.4rem] font-bold leading-snug tracking-tight transition-colors duration-200 ${i === 0 ? "text-white" : "text-white/25 hover:text-white"}`}>
                    {link.label}
                  </Link>
                </motion.div>
              ))}
            ...
            <p className="text-[11.5px] text-white/20 font-medium leading-relaxed max-w-[200px]">
          ...
          <div className="flex flex-col gap-5">
            ...
            <ul className="flex flex-col gap-2.5">
              {SERVICES.map((s) => (
                <li key={s.href}>
                  <Link href={s.href} className="text-[12.5px] font-semibold text-white/40 hover:text-white transition-colors leading-tight">{s.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <ul className="flex flex-col gap-2.5">
              {PRODUCTS.map((p) => (
                <li key={p.href}>
                  <Link href={p.href} className="text-[12.5px] font-semibold text-white/40 hover:text-white transition-colors leading-tight">{p.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <ul className="flex flex-col gap-2.5">
              {COMPANY.map((c) => (
                <li key={c.href}>
                  <Link href={c.href} className="text-[12.5px] font-semibold text-white/40 hover:text-white transition-colors">{c.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <ul className="flex flex-col gap-4">
              <li>
                <a href="mailto:devntomsolutions@gmail.com" className="flex items-start gap-2.5 group">
                  <Mail size={13} className="mt-0.5 flex-shrink-0 text-sky-500" />
                  <span className="text-[12px] font-semibold text-white/40 group-hover:text-white transition-colors leading-snug">devntomsolutions@gmail.com</span>
                </a>
              </li>
              <li>
                <a href="https://wa.me/923256036838" target="_blank" rel="noopener noreferrer" className="flex items-start gap-2.5 group">
              ...
              <li>
                <a href="tel:+966583408034" className="flex items-start gap-2.5 group">
                  <Phone size={13} className="mt-0.5 flex-shrink-0 text-sky-500" />
                  <span className="text-[12px] font-semibold text-white/40 group-hover:text-white transition-colors leading-snug">
                    +966 583 408034<br />
              ...
              <li className="flex items-start gap-2.5">
                ...
                <span className="text-[12px] font-semibold text-white/40 leading-snug">Lahore, Pakistan &<br />Saudi Arabia</span>
              </li>
            </ul>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {SOCIALS.map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/30 hover:text-white hover:border-sky-500/50 hover:bg-sky-500/10 transition-all duration-200">
                  {s.icon}
                </a>
              ))}
            </div>
      ...
      {/* ── CTA row ── */}
      <div className="relative z-10 px-6 lg:px-10 pb-6">
        <div className="flex flex-wrap items-center gap-4">
          <a href="mailto:devntomsolutions@gmail.com"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold text-white transition-all hover:scale-[1.03]"
            style={{ background: "linear-gradient(135deg, rgba(14,165,233,0.8), rgba(124,58,237,0.8))", boxShadow: "0 6px 24px rgba(14,165,233,0.25)" }}>
            Email Us <ArrowRight size={13} />
          </a>
          <a href="https://wa.me/923256036838" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold text-white border border-white/15 hover:border-white/30 hover:bg-white/[0.04] transition-all">
            WhatsApp
          </a>
        </div>
      </div>
      ...
      {/* ── Bottom bar ── */}
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-10 py-4 flex flex-wrap items-center justify-between gap-4 border-t border-white/[0.05]">
        <div className="flex items-center gap-5 flex-wrap">
          <p className="text-[11px] text-white/20 font-medium">DEVNTOM Solutions © {new Date().getFullYear()}</p>
          <Link href="/privacy-policy" className="text-[11px] text-white/15 hover:text-white/40 transition-colors font-medium">Privacy Policy</Link>
          <Link href="/terms-of-service" className="text-[11px] text-white/15 hover:text-white/40 transition-colors font-medium">Terms of Service</Link>
        </div>
